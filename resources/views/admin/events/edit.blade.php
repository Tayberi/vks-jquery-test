    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_start') ? 'is-invalid' : '' }}" type="text" name="date_start" id="date_start" value="{{ old('date_start', $event->date_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_start') ? 'is-invalid' : '' }}" type="text" name="time_start" id="time_start" value="{{ old('time_start', $event->time_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time_end') ? 'is-invalid' : '' }}" type="text" name="time_end" id="time_end" value="{{ old('time_end', $event->time_end) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('studios') ? 'is-invalid' : '' }}" name="studios[]" id="studios" multiple>
                    @foreach($studios as $id => $studio)
                        <option value="{{ $id }}" {{ (in_array($id, old('studios', [])) || $event->studios->contains($id)) ? 'selected' : '' }}>{{ $studio }}</option>
                    @endforeach
                </select>
